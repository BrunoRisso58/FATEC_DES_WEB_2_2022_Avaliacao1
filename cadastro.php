...
if (isset($_POST['name']) && isset($_POST['team']) && isset($_POST['age'])) {
    ...
    if(!file_exists("jogadores.txt")){
        $handle = fopen("jogadores.txt", "w");
    } else {
        $handle = fopen("jogadores.txt", "a");
    }
    $name = $_POST['name'];
    $team = $_POST['team'];
    $age = $_POST['age'];
    fwrite($handle, '<p style="margin: 0 30px">Nome: ' . $name . ' | Time: ' . $team . ' | Idade: ' . $age . '</p><br>');
    fflush($handle);
    fclose($handle);
}
...
<body>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
        ...
        <div class="field">
            <input type="text" class="input" name="team" placeholder="Time">
        </div>
        <div class="field">
            <input type="number" class="input" name="age" placeholder="Idade">
        </div>
        <div class="field">
